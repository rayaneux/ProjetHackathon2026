...
import { CheckCircle2, Send, Home, Users, Flag } from "lucide-react"
...
export default function Step6Bilan({ candidates, schoolCriteria, onReset, onComplete }: Props) {
      ...
      <Card className="shadow-lg border-brand-purple/20 overflow-hidden">
        ...
        <CardHeader className="pb-8 pt-8">
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <CardTitle className="text-4xl font-serif text-slate-900">Récapitulatif de campagne</CardTitle>
            <CardDescription className="text-xl max-w-2xl">
              <span className="font-semibold text-slate-800">{formationName || "Votre formation"}</span>
            </CardDescription>
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-8 px-12 pb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-brand-purple/5 p-6 rounded-2xl border border-brand-purple/20 flex flex-col items-center text-center">
              ...
              <p className="text-sm font-semibold uppercase tracking-wider text-brand-dark mb-1">Retours envoyés</p>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
            <h3 className="text-xl font-semibold text-slate-900 mb-6 font-serif">Promotion sélectionnée</h3>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-8 border-t border-slate-100">
            <Button
              ...
              onClick={onComplete}
            >
              <Flag className="w-5 h-5 mr-3" /> Terminer la campagne
            </Button>
            <button
              onClick={onReset}
              className="text-sm text-slate-400 hover:text-slate-600 transition-colors flex items-center gap-2"
            >
              <Home className="w-4 h-4" /> Retourner au tableau de bord sans clôturer
            </button>
          </div>
        </CardContent>
      </Card>
